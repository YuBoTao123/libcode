...
// template<typename P>
// torch::Tensor processScanEigen(const std::shared_ptr<P>& points,
...
template<typename P>
torch::Tensor processScanMat(const P& cloud,
                  std::vector<float> unproj_range, // distance info
                  std::vector<float> unproj_remissions, // intensity info
                 const cv::Mat& proj_range, // 修改为 cv::Mat
                 const cv::Mat& proj_xyz, // 修改为 cv::Mat
                 const cv::Mat& proj_remission, // 修改为 cv::Mat
                 const cv::Mat& proj_mask, // 修改为 cv::Mat
                 const std::vector<std::string>& residual_files,
                 bool use_residual,
                 int n_input_scans,
                 std::vector<float> sensor_img_means,
                 std::vector<float> sensor_img_stds) {

    // 获取点的数量
    int unproj_n_points = cloud->points.size();

    // 初始化张量
    torch::Tensor unproj_xyz = torch::full({kMaxPoints, 3}, -1.0, torch::kFloat32);
    unproj_xyz.slice(0, 0, unproj_n_points) = torch::from_blob(&cloud->front(), {unproj_n_points, 3}, torch::kFloat32);
    ...
    // 将 cv::Mat 转换为 Torch 张量
    torch::Tensor proj_range_tensor = torch::from_blob(proj_range.data, {proj_range.rows, proj_range.cols}, torch::kDouble).clone();
    torch::Tensor proj_xyz_tensor = torch::from_blob(proj_xyz.data, {proj_xyz.rows, proj_xyz.cols, 3}, torch::kDouble).clone();
    torch::Tensor proj_remission_tensor = torch::from_blob(proj_remission.data, {proj_remission.rows, proj_remission.cols}, torch::kDouble).clone();
