...
int main() {
    ...
    ICudaEngine* engine = runtime->deserializeCudaEngine(engineData.get(), engineSize, nullptr);
    if (!engine) {
    ...
    int inputIndex = engine->getBindingIndex("input"); // Replace with actual input name
    int outputIndex = engine->getBindingIndex("output"); // Replace with actual output name
    ...
    for (int i = 0; i < inputSize; ++i) {
        inputData[i] = static_cast<float>(rand()) / RAND_MAX; // Replace with your actual input data
    }
    ...
    cudaMemcpy(outputHostMem, outputDeviceMem, outputSize * sizeof(float), cudaMemcpyDeviceToHost);
